...
注：
我们阅读这本书的思路应是站在程序员的角度去看待它，即我们学习如何去优化程序，而不是站在硬件工程师的角度去深究他的电路及具体代码的实现。我们需要根据计算机内部的原理去写出符合他的工作原理的程序，这样就可以到达我们阅读此书的目的。
## 第一章：计算机系统漫游 ##

本书第一章快速的帮我们梳理了一下这本书所要学习的知识点，包括诸如：程序是怎样链接的、程序是怎样被翻译成计算机所能执行的代码、存储器的层次结构、进程的概念等等。通过阅读本章内容，我们将知道我们在接下来的章节中将学习哪些内容。在这里我不再赘述，接下来的内容将会讲解。


## 第二章：信息的表示和处理 ##
这一章里，我们首先将会学习到，在计算机系统里，我们以八位为一个字节，为什么？因为最开始设计cpu的时候，八位就能表示大多数字符。我们在计算机里会更多的用到十六进制表示信息，为什么？因为它相对于一长串的0和1而言，简洁了许多更容易让人去看。接下来，我们会看到，计算机里是如何实现加法、减法等等，不过就是移位进位。它这里只给出了代码和一点叙述但我还是拓展一下给出它的电路实现方便我们直观的理解。


如图：![](https://i.imgur.com/9Y1XgPW.png)

这是半加器，没有进位的处理，我们知道在二进制中1+1=0 1+0=1,所以说输出C(C是进位)就等于A乘B，输出S等于A异或B。

如果有进位的处理，我们就需要全加器，如图：

![](https://i.imgur.com/F3BL4fA.png)

这时我们就需要考虑后一位的进Cin,所以这时产生的进位Cout和结果S就等于图中的等式。

紧接着，书中告诉了我们如何表示各种数字信息，如补码、有符号数、无符号数、浮点数等等。
知道了这些，我们就可以完整的知道计算机内部的计算是怎么进行的。

在这一章中我们要掌握的点就是知道计算机内部是如何实现各种运算及如何表示各种信息。

## 第三章：程序的机器级表示和处理 ##
在这一章中，我们将会学习我们所编写的代码是在计算机里是如何表示成计算机可识别的机器码。

下面我给出两张图：

![](https://i.imgur.com/YQb2tJ8.png)


![](https://i.imgur.com/UhY8qiw.png)
...
![](https://i.imgur.com/zeNOJHA.png)

首先我们在R2寄存器的低八位设置为1，接下来将R2的高八位设置为0
